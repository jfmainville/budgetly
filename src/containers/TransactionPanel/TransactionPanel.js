...
export class TransactionPanel extends Component {
	...
	handleMonthSelectionPrevious = () => {
		let activeMonth = moment(this.state.activeMonth, "YYYY-MM")
			.add(-1, "month")
			.format("YYYY-MM");
		this.setState({
...
TransactionPanel.propTypes = {
	...
	createTransactionRow: PropTypes.func
};
...
const mapDispatchToProps = dispatch => {
	return {
		...
		createTransactionRow: (rowId, transactionId) =>
			dispatch(actions.createTransactionRow(rowId, transactionId))
	};
};
